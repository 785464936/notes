...
![方框图等效变换|800](assets/fig/测控-1.jpg) 
...
- 输入为单位阶跃信号 $X_{i}(s) = \frac{1}{s}$ 时，稳态偏差 $$\varepsilon_{ss} = \lim_{ s \to 0 }s \cdot E(s) = \lim_{ s \to 0 } s\cdot \frac{X_{i}(s)}{1+G(s)H(s)} = \lim_{ s \to 0 } \frac{1}{1+G(s)H(s)} = \frac{1}{1+K_{\mathrm{p}}}$$ 式中 $$K_{\mathrm{p}} = \lim_{ s \to 0 } G(s)H(s) = \lim_{ s \to 0 } \frac{K}{s^{\nu}}G_{o}(s) = \lim_{ s \to 0 } \frac{K}{s^{\nu}}$$称 $K_{\mathrm{p}}$ 为 **位置无偏系数** %% P105-P107 %%
    - 对 0 型系统，$K_{\mathrm{p}} = lim_{ s \to 0 } \frac{K}{s^{\nu}} = K,\varepsilon_{ss} = \frac{1}{1+K}$，称之为 **位置有差系统**，且 $K$ 越大，$\varepsilon_{ss}$ 越小。
    - 对于 I、II 型系统，$K_{\mathrm{p}} = lim_{ s \to 0 } \frac{K}{s^{\nu}} = \infty, \varepsilon_{ss} = 0$，称之为 **位置无差系统**。
- 输入为单位斜坡信号时，有 $$x_{i}(t) = t,\quad X_{i}(s) = \frac{1}{s^2}$$此时$$\varepsilon_{ss} = \lim_{ s \to 0 }s \cdot E(s) = \lim_{ s \to 0 } s\cdot \frac{X_{i}(s)}{1+G(s)H(s)} = \lim_{ s \to 0 } s \cdot \frac{{1/s^2}}{1+G(s)H(s)} = \lim_{ s \to 0 } \frac{1}{sG(s)H(s)} = \frac{1}{K_{\mathrm{v}}}$$式中 $$K_{\mathrm{v}} = \lim_{ s \to 0 } sG(s)H(s) = \lim_{ s \to 0 } \frac{sKG_{o}(s)}{s^{\nu}} = \lim_{ s \to 0 } \frac{K}{s^{\nu - 1}}$$称 $K_{\mathrm{v}}$ **为速度无偏系数**。
    - 对 0 型系统，有 $K_{\mathrm{v}} = \lim_{ s \to 0 }s \cdot K  = 0 \quad \varepsilon_{ss} = \frac{1}{K_{\mathrm{v}}} = \infty$
    - 对 I 型系统，有 $K_{\mathrm{v}} = \lim_{ s \to 0 } \frac{K}{s^0} = K \quad \varepsilon_{ss} = \frac{1}{K_{\mathrm{v}}} = \frac{1}{K}$
    - 对 II 型系统，有 $K_{\mathrm{v}} = \lim_{ s \to 0 } \frac{K}{s} = \infty \quad \varepsilon_{ss} = \frac{1}{K_{\mathrm{v}}} = 0$ 
    - 上述分析说明：0 型系统不能跟随斜坡输入，因为其稳态偏差为 $\infty$；I 型系统可以跟随，但存在稳态偏差，可以通过增大 $K$ 值来减小偏差；II 型或高于 II 型的系统，对斜坡输入响应的稳态是无差的。
- 输入为加速度信号时，有 $$x_{i}(t) = \frac{1}{2} t^2 \quad X_{i}(s) = \frac{1}{s^3}$$ 此时 $$\varepsilon_{ss} = \lim_{ s \to 0 }s \cdot E(s) = \lim_{ s \to 0 } s\cdot \frac{X_{i}(s)}{1+G(s)H(s)} = \lim_{ s \to 0 } s \cdot \frac{{1/s^3}}{1+G(s)H(s)} = \lim_{ s \to 0 } \frac{1}{s^2G(s)H(s)} = \frac{1}{K_{\mathrm{a}}}$$式中 $$K_{\mathrm{a}} = \lim_{ s \to 0 } s^2G(s)H(s) = \lim_{ s \to 0 } \frac{s^2 K G_{o}(s)}{s^{\nu}} = \lim_{ s \to 0 } \frac{K}{s^{\nu - 2}}$$称 $K_{\mathrm{a}}$ 为 **加速度无偏系数**。
    - 对 0、I 型系统，有 $K_{\mathrm{a}} = \lim_{ s \to 0 } \frac{K}{s^{\nu - 2}} \quad \varepsilon_{ss} = \frac{1}{K_{\mathrm{a}}} = \infty$
    - 对 II 型系统，有 $K_{\mathrm{a}} = K \quad \varepsilon_{ss} = \frac{1}{K}$
    - 可见，输入为加速度信号时，0、I 型系统不能跟随，II 型系统为有差系统，III 型或高于 III 型的系统无差。
- ![稳态偏差|800](assets/fig/测控-11.png)
...
### 7. 系统的校正
$\xi - ,\omega_{n}\uparrow,M_{\mathrm{p}}-,t_{r}\downarrow,t_{p}\downarrow,t_{s}\downarrow$
$\omega_{n} - ,\xi\downarrow,t_{r}\downarrow,t_{p}\downarrow,M_{\mathrm{p}}\uparrow,t_{s}\uparrow (\xi \lt 0.707)$
$\varphi(\omega_{c}) = -90-\arctan T\omega_{c}-\cdots$
$\gamma = 180+\varphi(\omega_{c})$
$\angle G(j\omega) = -90-\arctan T\omega_{g}-\cdots = -180 \to \omega_{g}$
$K_{g} = -20\lg |G(j\omega_{g})H(j\omega_{g})|$
$K_{g} (dB) = 20\lg K_{g}$
